...
class JiTBlock(nn.Module):
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, attn_drop=0.0, proj_drop=0.0):
        ...
        #                                  attn_drop=attn_drop, proj_drop=proj_drop)
        self.cross_attn_sar = CrossAttention(hidden_size, num_heads=num_heads, qkv_bias=True, qk_norm=True,
                                             attn_drop=attn_drop, proj_drop=proj_drop)
        self.cross_attn_hint = CrossAttention(hidden_size, num_heads=num_heads, qkv_bias=True, qk_norm=True,
                                              attn_drop=attn_drop, proj_drop=proj_drop)
        self.norm_ca_gate = RMSNorm(hidden_size, eps=1e-6)
    ...
    @torch.compile
    def forward(self, x, c, feat_rope=None, sar_tokens=None, hint_tokens=None, sar_rope=None, hint_rope=None):
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(c).chunk(6, dim=-1)
        x = x + gate_msa.unsqueeze(1) * self.attn(modulate(self.norm1(x), shift_msa, scale_msa), rope=feat_rope)
        sar_ca = None
        hint_ca = None
        ca_query = self.norm_ca(x)
        if sar_tokens is not None:
            sar_ca = self.cross_attn_sar(ca_query, sar_tokens, rope=feat_rope, context_rope=sar_rope)
        if hint_tokens is not None:
            hint_ca = self.cross_attn_hint(ca_query, hint_tokens, rope=feat_rope, context_rope=hint_rope)

        if sar_ca is not None and hint_ca is not None:
            gate_context = c.unsqueeze(1).expand(-1, x.shape[1], -1)
            gate_in = torch.cat([self.norm_ca_gate(x), sar_ca, hint_ca, gate_context], dim=-1)
            gate = torch.sigmoid(self.ca_gate(gate_in))
            ca_out = gate * sar_ca + (1.0 - gate) * hint_ca
            x = x + self.ca_scale * ca_out
        elif sar_ca is not None:
            x = x + self.ca_scale * sar_ca
        elif hint_ca is not None:
            x = x + self.ca_scale * hint_ca
